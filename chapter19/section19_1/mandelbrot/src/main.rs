...
fn main() {
  ...
  let upper_left = parse_complex(&args[3])
    .expect("error parsing upple left corner point");
  ...
  // let rows_per_band = bounds.1 /thread + 1;
  {
    ...
    bands.into_par_iter()
      .for_each(|(i,band)| {
        ...
        let band_upperleft = pixel_to_point(bounds,(0, top), upper_left, lower_right);
        let band_lower_right = pixel_to_point(bounds, (bounds.0, top + 1), upper_left, lower_right);
        render(band, band_bounds, band_upperleft, band_lower_right);
      });
...
fn pixel_to_point(bound : (usize, usize),
                  pixel: (usize, usize),
...
fn render(pixels: &mut [u8],
  ...
  for row in 0..bounds.1 {
    for colum in 0..bounds.0 {
      let point = pixel_to_point(bounds, (colum,row),
                                upper_left,lower_right);
      pixels[row* bounds.0 + colum] = 
